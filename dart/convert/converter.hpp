...
class ConverterCls : public StreamTransformerBaseCls<S, T> {
public:

     ConverterCls();
    template<typename SS, typename ST, typename TS, typename TT>
 static Converter<TS, TT> castFrom(Converter<SS, ST> source);

    virtual T convert(S input);
    template<typename TT>
 virtual Converter<S, TT> fuse(Converter<T, TT> other);
...
template<typename S, typename T>
using Converter = std::shared_ptr<ConverterCls<S, T>>;

template<typename S, typename M, typename T>
class _FusedConverterCls : public ConverterCls<S, T> {
public:
...
template<typename S, typename M, typename T>
using _FusedConverter = std::shared_ptr<_FusedConverterCls<S, M, T>>;

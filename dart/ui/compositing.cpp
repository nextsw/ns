...
bool _EngineLayerWrapperCls::_debugCheckNotUsedAsOldLayer() {
    assert(!_debugWasUsedAsOldLayer, __sf("Layer %s was previously used as oldLayer.\nOnce a layer is used as oldLayer, it may not be used again. Instead, after calling one of the SceneBuilder.push* methods and passing an oldLayer to it, use the layer returned by the method as oldLayer in subsequent frames.", runtimeType));
    return true;
}
...
bool SceneBuilderCls::_debugCheckUsedOnce(EngineLayer layer, String usage) {
    assert([=] () {
        assert(!_usedLayers->containsKey(layer), __sf("Layer %s already used.\nThe layer is already being used as %s in this scene.\nA layer may only be used once in a given scene.", layer->runtimeType, _usedLayers[layer]));
...
bool SceneBuilderCls::_debugCheckCanBeUsedAsOldLayer(_EngineLayerWrapper layer, String methodName) {
    assert([=] () {
        ...
        assert(_debugCheckUsedOnce(layer, __sf("oldLayer in %s", methodName)));

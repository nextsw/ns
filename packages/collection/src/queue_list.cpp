...
void QueueListCls<E>::length(int value) {
    if ( < 0) {
        throw make<RangeErrorCls>(__sf("Length %s may not be negative.", value));
    }
    if (value > length() && !is<E>(nullptr)) {
        throw make<UnsupportedErrorCls>(__sf("The length can only be increased when the element type is nullable, but the current element type is `%s`.", E));
    }
...
E QueueListCls<E>::[](int index) {
    if ( < 0 || index >= length()) {
        throw make<RangeErrorCls>(__sf("Index %s must be in the range [0..%s).", index, length()));
    }

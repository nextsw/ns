...
double SliverGridGeometry::trailingScrollOffset() {
    return scrollOffset + mainAxisExtent;
}

BoxConstraints SliverGridGeometry::getBoxConstraints(SliverConstraints constraints) {
    return constraints.asBoxConstraints(mainAxisExtent, mainAxisExtent, crossAxisExtent);
}

String SliverGridGeometry::toString() {
    List<String> properties = ;
    return "SliverGridGeometry(${properties.join(', ')})";
}

SliverGridRegularTileLayout::SliverGridRegularTileLayout(double childCrossAxisExtent, double childMainAxisExtent, int crossAxisCount, double crossAxisStride, double mainAxisStride, bool reverseCrossAxis) {
    {
...
int SliverGridRegularTileLayout::getMinChildIndexForScrollOffset(double scrollOffset) {
    return mainAxisStride > precisionErrorTolerance? crossAxisCount * (scrollOffset ~/ mainAxisStride) : 0;
}

int SliverGridRegularTileLayout::getMaxChildIndexForScrollOffset(double scrollOffset) {
...
SliverGridGeometry SliverGridRegularTileLayout::getGeometryForChildIndex(int index) {
    double crossAxisStart = (index % crossAxisCount) * crossAxisStride;
    return SliverGridGeometry((index ~/ crossAxisCount) * mainAxisStride, _getOffsetFromStartInCrossAxis(crossAxisStart), childMainAxisExtent, childCrossAxisExtent);
}

double SliverGridRegularTileLayout::computeMaxScrollOffset(int childCount) {
...
double SliverGridRegularTileLayout::_getOffsetFromStartInCrossAxis(double crossAxisStart) {
    if (reverseCrossAxis) {
...
SliverGridDelegateWithFixedCrossAxisCount::SliverGridDelegateWithFixedCrossAxisCount(double childAspectRatio, int crossAxisCount, double crossAxisSpacing, double mainAxisExtent, double mainAxisSpacing) {
    {
        assert(crossAxisCount != nullptr && crossAxisCount > 0);
...
SliverGridLayout SliverGridDelegateWithFixedCrossAxisCount::getLayout(SliverConstraints constraints) {
    ...
    return SliverGridRegularTileLayout(crossAxisCount, childMainAxisExtent + mainAxisSpacing, childCrossAxisExtent + crossAxisSpacing, childMainAxisExtent, childCrossAxisExtent, axisDirectionIsReversed(constraints.crossAxisDirection));
}

bool SliverGridDelegateWithFixedCrossAxisCount::shouldRelayout(SliverGridDelegateWithFixedCrossAxisCount oldDelegate) {
...
bool SliverGridDelegateWithFixedCrossAxisCount::_debugAssertIsValid() {
...
SliverGridDelegateWithMaxCrossAxisExtent::SliverGridDelegateWithMaxCrossAxisExtent(double childAspectRatio, double crossAxisSpacing, double mainAxisExtent, double mainAxisSpacing, double maxCrossAxisExtent) {
    {
...
SliverGridLayout SliverGridDelegateWithMaxCrossAxisExtent::getLayout(SliverConstraints constraints) {
...
bool SliverGridDelegateWithMaxCrossAxisExtent::shouldRelayout(SliverGridDelegateWithMaxCrossAxisExtent oldDelegate) {
...
bool SliverGridDelegateWithMaxCrossAxisExtent::_debugAssertIsValid(double crossAxisExtent) {
...
String SliverGridParentData::toString() {
    return "crossAxisOffset=$crossAxisOffset; ${super.toString()}";
}

RenderSliverGrid::RenderSliverGrid(Unknown, SliverGridDelegate gridDelegate) {
    {
...
void RenderSliverGrid::setupParentData(RenderObject child) {
    if (child.parentData is! SliverGridParentData) {
        child.parentData = SliverGridParentData();
    }
}

SliverGridDelegate RenderSliverGrid::gridDelegate() {
    return _gridDelegate;
}

void RenderSliverGrid::gridDelegate(SliverGridDelegate value) {
...
double RenderSliverGrid::childCrossAxisPosition(RenderBox child) {
    SliverGridParentData childParentData = (;
    return childParentData.crossAxisOffset!;
}

void RenderSliverGrid::performLayout() {
    ...
    geometry = SliverGeometry(estimatedTotalExtent, paintExtent, estimatedTotalExtent, cacheExtent, estimatedTotalExtent > paintExtent || constraints.scrollOffset > 0.0 || constraints.overlap != 0.0);

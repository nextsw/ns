...
String HitTestEntryCls<T>::toString() {
    return __sf("%s(%s)", describeIdentity(this), target);
}
...
void HitTestResultCls::pushTransform(Matrix4 transform) {
    ...
    assert(_debugVectorMoreOrLessEquals(transform->getRow(2), make<Vector4Cls>(0, 0, 1, 0)) && _debugVectorMoreOrLessEquals(transform->getColumn(2), make<Vector4Cls>(0, 0, 1, 0)), __sf("The third row and third column of a transform matrix for pointer events must be Vector4(0, 0, 1, 0) to ensure that a transformed point is directly under the pointing device. Did you forget to run the paint matrix through PointerEvent.removePerspectiveTransform? The provided matrix is:\n%s", transform));
...
String HitTestResultCls::toString() {
    return __sf("HitTestResult(%s)", _path->isEmpty()? __s("<empty path>") : _path->join(__s(", ")));
}

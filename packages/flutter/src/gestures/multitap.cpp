...
_TapGestureCls::_TapGestureCls(PointerEvent event, MultiTapGestureRecognizer gestureRecognizer, DeviceGestureSettings gestureSettings, Duration longTapDelay) : _TapTracker(as<PointerDownEvent>(event), GestureBindingCls::instance->gestureArena->add(event->pointer, gestureRecognizer), kDoubleTapMinTime) {
    {
        _lastPosition = OffsetPairCls->fromEventPosition(event);
    }
    {

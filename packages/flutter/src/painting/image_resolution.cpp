...
Future<Map<String, List<String>>> AssetImageCls::_manifestParser(String jsonData) {
    ...
    Map<String, Object> parsedJson = as<Map<String, Object>>(json->decode(jsonData));
    Iterable<String> keys = parsedJson->keys();
    Map<String, List<String>> map1 = make<MapCls<>>();for (String key : keys) {    ;}{    map1.set(key, <String>from(as<List<Object>>(parsedJson[key])));}Map<String, List<String>> parsedManifest = list1;

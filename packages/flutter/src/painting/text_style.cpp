...
void TextStyleCls::debugFillProperties(String prefix, DiagnosticPropertiesBuilder properties) {
    ...
    styles->add(make<DoublePropertyCls>("${prefix}letterSpacing", letterSpacingnullptr));
    styles->add(make<DoublePropertyCls>("${prefix}wordSpacing", wordSpacingnullptr));
    styles->add(<TextBaseline>make<EnumPropertyCls>("${prefix}baseline", textBaselinenullptr));
    styles->add(make<DoublePropertyCls>("${prefix}height", height"x", nullptr));
    styles->add(<TextLeadingDistribution>make<EnumPropertyCls>("${prefix}leadingDistribution", leadingDistributionnullptr));
    styles->add(<Locale>make<DiagnosticsPropertyCls>("${prefix}locale", localenullptr));
    styles->add(<Paint>make<DiagnosticsPropertyCls>("${prefix}foreground", foregroundnullptr));
    styles->add(<Paint>make<DiagnosticsPropertyCls>("${prefix}background", backgroundnullptr));
    if (decoration != nullptr || decorationColor != nullptr || decorationStyle != nullptr || decorationThickness != nullptr) {
        ...
        styles->add(make<ColorPropertyCls>("${prefix}decorationColor", decorationColornullptr, DiagnosticLevelCls::fine));
        if (decorationColor != nullptr) {
            decorationDescription->add("$decorationColor");
        }
        styles->add(<TextDecoration>make<DiagnosticsPropertyCls>("${prefix}decoration", decorationnullptr, DiagnosticLevelCls::hidden));
        if (decoration != nullptr) {
            decorationDescription->add("$decoration");
        }
        assert(decorationDescription->isNotEmpty);
        styles->add(make<MessagePropertyCls>("${prefix}decoration", decorationDescription->join(" ")));
        styles->add(make<DoublePropertyCls>("${prefix}decorationThickness", decorationThickness"x", nullptr));
    }
    bool styleSpecified = styles->any([=] (DiagnosticsNode n) {
    !n->isFiltered(DiagnosticLevelCls::info);
});
    properties->add(<bool>make<DiagnosticsPropertyCls>("${prefix}inherit", inherit(!styleSpecified && inherit)? DiagnosticLevelCls::fine : DiagnosticLevelCls::info));
    styles->forEach(properties->add);
    if (!styleSpecified) {
        properties->add(make<FlagPropertyCls>("inherit"inherit, "$prefix<all styles inherited>", "$prefix<no style specified>"));
    }
    styles->add(<TextOverflow>make<EnumPropertyCls>("${prefix}overflow", overflownullptr));
}

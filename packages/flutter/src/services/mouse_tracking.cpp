...
void MouseTrackerAnnotationCls::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    Map<String, std::function<void ()>> map1 = make<MapCls<>>();map1.set(__s("enter"), onEnter);map1.set(__s("exit"), onExit);properties->add(<std::function<void ()>>make<FlagsSummaryCls>(__s("callbacks"), list1, __s("<none>")));

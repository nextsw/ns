...
String CustomPainterCls::toString() {
    return __sf("%s(%s", describeIdentity(this), _repaint?->toString() | __s(")"));
}
...
void RenderCustomPaintCls::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties->add(make<MessagePropertyCls>(__s("painter"), __sf("%s", painter())));
    properties->add(make<MessagePropertyCls>(__s("foregroundPainter"), __sf("%s", foregroundPainter()), foregroundPainter() != nullptr? DiagnosticLevelCls::info : DiagnosticLevelCls::fine));
        ...
        for (;  < newChildSemantics!->length(); i += 1) {
            ...
            if (child->key != nullptr) {
                if (keys->containsKey(child->key)) {
                    information->add(make<ErrorDescriptionCls>(__sf("- duplicate key %s found at position %s", child->key, i)));
                }
                keys[child->key!] = i;
            }
        }

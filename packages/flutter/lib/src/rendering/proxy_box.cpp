...
Size RenderAspectRatioCls::_applyAspectRatio(BoxConstraints constraints) {
    ...
    assert([=] () {
        if (!constraints->hasBoundedWidth() && !constraints->hasBoundedHeight()) {
            throw make<FlutterErrorCls>(__sf("%s has unbounded constraints.\nThis %s was given an aspect ratio of %s but was given both unbounded width and unbounded height constraints. Because both constraints were unbounded, this render object doesn't know how much size to consume.", runtimeType, runtimeType, aspectRatio()));
        }
        return true;
    }());
...
Size RenderFittedBoxCls::computeDryLayout(BoxConstraints constraints) {
    if (child != nullptr) {
        ...
        if (invalidChildSize) {
            assert(debugCannotComputeDryLayout(__sf("Child provided invalid size of %s.", childSize)));
            return SizeCls::zero;
        }
        ;
    } else {
...
;
            ...
            }            properties->add(make<PercentPropertyCls>(__s("metrics"), fraction, __s("useful"), __sf("%s bad vs %s good", debugSymmetricPaintCount(), debugAsymmetricPaintCount())));
...
void RenderFollowerLayerCls::paint(PaintingContext context, Offset offset) {
    ...
    assert(link()->leaderSize != nullptr || (link()->leader() == nullptr || leaderAnchor() == AlignmentCls::topLeft), __sf("%s: layer is linked to %s but a valid leaderSize is not set. leaderSize is required when leaderAnchor is not Alignment.topLeft (current value is %s).", link(), link()->leader(), leaderAnchor()));

...
void LayerCls::dispose() {
    ...
    assert([=] () {
        assert(_refCount == 0, "Do not directly call dispose on a $runtimeType. Instead, use createHandle and LayerHandle.dispose.");
        _debugDisposed = true;
        return true;
    }());
...
ContainerLayer LayerCls::parent() {
    return ((ContainerLayer)super->parent);
}
...
bool LayerCls::findAnnotationstemplate<typename S : Object> (Offset localPosition, bool onlyFirst, AnnotationResult<S> result) {
    return false;
}

S LayerCls::findtemplate<typename S : Object> (Offset localPosition) {
...
AnnotationResult<S> LayerCls::findAllAnnotationstemplate<typename S : Object> (Offset localPosition) {
    AnnotationResult<S> result = <S>make<AnnotationResultCls>();
...
void LayerCls::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties->add(<Object>make<DiagnosticsPropertyCls>("owner", ownerparent != nullptr? DiagnosticLevelCls::hidden : DiagnosticLevelCls::info, nullptr));
    ...
    if (_engineLayer != nullptr) {
        properties->add(<String>make<DiagnosticsPropertyCls>("engine layer", describeIdentity(_engineLayer)));
    }
    properties->add(<int>make<DiagnosticsPropertyCls>("handles", debugHandleCount));
}
...
template<typename T : Layer> LayerHandleCls<T>::LayerHandleCls(T _layer) {
    {
        if (_layer != nullptr) {
            _layer!->_refCount = 1;
        }
...
template<typename T : Layer> T LayerHandleCls<T>::layer() {
    return _layer;
}

template<typename T : Layer> void LayerHandleCls<T>::layer(T layer) {
...
template<typename T : Layer> String LayerHandleCls<T>::toString() {
...
void PictureLayerCls::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super->debugFillProperties(properties);
    properties->add(<Rect>make<DiagnosticsPropertyCls>("paint bounds", canvasBounds));
    properties->add(<String>make<DiagnosticsPropertyCls>("picture", describeIdentity(_picture)));
    properties->add(<String>make<DiagnosticsPropertyCls>("raster cache hints", "isComplex = $isComplexHint, willChange = $willChangeHint"));
}

bool PictureLayerCls::findAnnotationstemplate<typename S : Object> (Offset localPosition, bool onlyFirst, AnnotationResult<S> result) {
    return false;
}
...
bool TextureLayerCls::findAnnotationstemplate<typename S : Object> (Offset localPosition, bool onlyFirst, AnnotationResult<S> result) {
    return false;
}
...
bool PerformanceOverlayLayerCls::findAnnotationstemplate<typename S : Object> (Offset localPosition, bool onlyFirst, AnnotationResult<S> result) {
    return false;
}
...
bool ContainerLayerCls::findAnnotationstemplate<typename S : Object> (Offset localPosition, bool onlyFirst, AnnotationResult<S> result) {
...
bool OffsetLayerCls::findAnnotationstemplate<typename S : Object> (Offset localPosition, bool onlyFirst, AnnotationResult<S> result) {
...
void OffsetLayerCls::addToScene(SceneBuilder builder) {
    engineLayer = builder->pushOffset(offset->dx, offset->dy((OffsetEngineLayer)_engineLayer));
    addChildrenToScene(builder);
    builder->pop();
}
...
bool ClipRectLayerCls::findAnnotationstemplate<typename S : Object> (Offset localPosition, bool onlyFirst, AnnotationResult<S> result) {
...
void ClipRectLayerCls::addToScene(SceneBuilder builder) {
    ...
    if (enabled) {
        engineLayer = builder->pushClipRect(clipRect!clipBehavior, ((ClipRectEngineLayer)_engineLayer));
    } else {
        engineLayer = nullptr;
    }
    addChildrenToScene(builder);
...
void ClipRectLayerCls::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super->debugFillProperties(properties);
    properties->add(<Rect>make<DiagnosticsPropertyCls>("clipRect", clipRect));
...
bool ClipRRectLayerCls::findAnnotationstemplate<typename S : Object> (Offset localPosition, bool onlyFirst, AnnotationResult<S> result) {
...
void ClipRRectLayerCls::addToScene(SceneBuilder builder) {
    ...
    if (enabled) {
        engineLayer = builder->pushClipRRect(clipRRect!clipBehavior, ((ClipRRectEngineLayer)_engineLayer));
    } else {
        engineLayer = nullptr;
    }
    addChildrenToScene(builder);
...
void ClipRRectLayerCls::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super->debugFillProperties(properties);
    properties->add(<RRect>make<DiagnosticsPropertyCls>("clipRRect", clipRRect));
    properties->add(<Clip>make<DiagnosticsPropertyCls>("clipBehavior", clipBehavior));
}
...
bool ClipPathLayerCls::findAnnotationstemplate<typename S : Object> (Offset localPosition, bool onlyFirst, AnnotationResult<S> result) {
...
void ClipPathLayerCls::addToScene(SceneBuilder builder) {
    ...
    if (enabled) {
        engineLayer = builder->pushClipPath(clipPath!clipBehavior, ((ClipPathEngineLayer)_engineLayer));
    } else {
        engineLayer = nullptr;
    }
    addChildrenToScene(builder);
...
void ColorFilterLayerCls::addToScene(SceneBuilder builder) {
    ...
    engineLayer = builder->pushColorFilter(colorFilter!((ColorFilterEngineLayer)_engineLayer));
    addChildrenToScene(builder);
    builder->pop();
}
...
void ColorFilterLayerCls::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super->debugFillProperties(properties);
    properties->add(<ColorFilter>make<DiagnosticsPropertyCls>("colorFilter", colorFilter));
}
...
void ImageFilterLayerCls::addToScene(SceneBuilder builder) {
    ...
    engineLayer = builder->pushImageFilter(imageFilter!((ImageFilterEngineLayer)_engineLayer));
    addChildrenToScene(builder);
    builder->pop();
}
...
void TransformLayerCls::addToScene(SceneBuilder builder) {
    ...
    engineLayer = builder->pushTransform(_lastEffectiveTransform!->storage((TransformEngineLayer)_engineLayer));
    addChildrenToScene(builder);
    builder->pop();
}

bool TransformLayerCls::findAnnotationstemplate<typename S : Object> (Offset localPosition, bool onlyFirst, AnnotationResult<S> result) {
...
void TransformLayerCls::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super->debugFillProperties(properties);
    properties->add(make<TransformPropertyCls>("transform", transform));
}
...
void OpacityLayerCls::addToScene(SceneBuilder builder) {
    ...
    if (enabled &&  < 255) {
        ...
        engineLayer = builder->pushOpacity(realizedAlphaoffset, ((OpacityEngineLayer)_engineLayer));
    } else {
...
void OpacityLayerCls::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super->debugFillProperties(properties);
    properties->add(make<IntPropertyCls>("alpha", alpha));
}
...
void ShaderMaskLayerCls::addToScene(SceneBuilder builder) {
    ...
    engineLayer = builder->pushShaderMask(shader!, maskRect!, blendMode!((ShaderMaskEngineLayer)_engineLayer));
    addChildrenToScene(builder);
    builder->pop();
}
...
void ShaderMaskLayerCls::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super->debugFillProperties(properties);
    properties->add(<Shader>make<DiagnosticsPropertyCls>("shader", shader));
    properties->add(<Rect>make<DiagnosticsPropertyCls>("maskRect", maskRect));
    properties->add(<BlendMode>make<EnumPropertyCls>("blendMode", blendMode));
}
...
void BackdropFilterLayerCls::addToScene(SceneBuilder builder) {
    ...
    engineLayer = builder->pushBackdropFilter(filter!blendMode, ((BackdropFilterEngineLayer)_engineLayer));
    addChildrenToScene(builder);
    builder->pop();
}
...
void BackdropFilterLayerCls::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super->debugFillProperties(properties);
    properties->add(<ImageFilter>make<DiagnosticsPropertyCls>("filter", filter));
    properties->add(<BlendMode>make<EnumPropertyCls>("blendMode", blendMode));
}
...
bool PhysicalModelLayerCls::findAnnotationstemplate<typename S : Object> (Offset localPosition, bool onlyFirst, AnnotationResult<S> result) {
    if (!clipPath!->contains(localPosition)) {
        return false;
    }
    return super-><S>findAnnotations(result, localPositiononlyFirst);
...
void PhysicalModelLayerCls::addToScene(SceneBuilder builder) {
    ...
    if (enabled) {
        engineLayer = builder->pushPhysicalShape(clipPath!, elevation!, color!, shadowColor, clipBehavior, ((PhysicalShapeEngineLayer)_engineLayer));
    } else {
        engineLayer = nullptr;
    }
    addChildrenToScene(builder);
...
bool LeaderLayerCls::findAnnotationstemplate<typename S : Object> (Offset localPosition, bool onlyFirst, AnnotationResult<S> result) {
    return super-><S>findAnnotations(result, localPosition - offsetonlyFirst);
}
...
void LeaderLayerCls::addToScene(SceneBuilder builder) {
    ...
    if (offset != OffsetCls::zero) {
        engineLayer = builder->pushTransform(Matrix4Cls->translationValues(offset->dx, offset->dy, 0.0)->storage((TransformEngineLayer)_engineLayer));
    }
    addChildrenToScene(builder);
    if (offset != OffsetCls::zero) {
        builder->pop();
...
bool FollowerLayerCls::findAnnotationstemplate<typename S : Object> (Offset localPosition, bool onlyFirst, AnnotationResult<S> result) {
...
void FollowerLayerCls::addToScene(SceneBuilder builder) {
    ...
    if (_lastTransform != nullptr) {
        ...
        engineLayer = builder->pushTransform(_lastTransform!->storage((TransformEngineLayer)_engineLayer));
        addChildrenToScene(builder);
        builder->pop();
    } else {
        _lastOffset = nullptr;
        Matrix4 matrix = Matrix4Cls->translationValues(unlinkedOffset!->dx, unlinkedOffset!->dy, .0);
        engineLayer = builder->pushTransform(matrix->storage((TransformEngineLayer)_engineLayer));
        addChildrenToScene(builder);
        builder->pop();
    }
    _inverseDirty = true;
...
void FollowerLayerCls::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super->debugFillProperties(properties);
    properties->add(<LayerLink>make<DiagnosticsPropertyCls>("link", link));
    properties->add(make<TransformPropertyCls>("transform", getLastTransform()nullptr));
}
...
template<typename T : Object> AnnotatedRegionLayerCls<T>::AnnotatedRegionLayerCls(Offset offset, bool opaque, Size size, T value) {
    {
        assert(value != nullptr);
...
template<typename T : Object> bool AnnotatedRegionLayerCls<T>::findAnnotationstemplate<typename S : Object> (Offset localPosition, bool onlyFirst, AnnotationResult<S> result) {
...
template<typename T : Object> void AnnotatedRegionLayerCls<T>::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super->debugFillProperties(properties);
    properties->add(<T>make<DiagnosticsPropertyCls>("value", value));
    properties->add(<Size>make<DiagnosticsPropertyCls>("size", sizenullptr));
    properties->add(<Offset>make<DiagnosticsPropertyCls>("offset", offsetnullptr));
    properties->add(<bool>make<DiagnosticsPropertyCls>("opaque", opaquefalse));
}

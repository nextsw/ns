...
void LayerCls::dispose() {
    ...
    assert([=] () {
        assert(_refCount == 0, __s("Do not directly call dispose on a $runtimeType. Instead, use createHandle and LayerHandle.dispose."));
        _debugDisposed = true;
        return true;
    }());
...
void LayerCls::markNeedsAddToScene() {
    assert(!_debugMutationsLocked);
    assert(!alwaysNeedsAddToScene(), __s("$runtimeType with alwaysNeedsAddToScene set called markNeedsAddToScene.\nThe layer's alwaysNeedsAddToScene is set to true, and therefore it should not call markNeedsAddToScene."));
...
String LayerCls::toStringShort() {
    return __s("${super.toStringShort()}${ owner == null ? " DETACHED" : ""}");
}
...
String LayerLinkCls::toString() {
    return __s("${describeIdentity(this)}(${ _leader != null ? "<linked>" : "<dangling>" })");
}
...
void AnnotatedRegionLayerCls<T>::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties->add(<Offset>make<DiagnosticsPropertyCls>(__s("offset"), offsetnullptr));
    properties->add(<bool>make<DiagnosticsPropertyCls>(__s("opaque"), opaquefalse));
}

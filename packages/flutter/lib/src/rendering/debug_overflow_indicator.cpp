...
void DebugOverflowIndicatorMixinCls::paintOverflowIndicator(Rect childRect, Rect containerRect, PaintingContext context, Offset offset, List<DiagnosticsNode> overflowHints) {
    ...
    for (_OverflowRegionData region : overflowRegions) {
        ...
        TextSpan textSpan = ((TextSpan)_indicatorLabel[region->side->index]->text);
...
List<_OverflowRegionData> DebugOverflowIndicatorMixinCls::_calculateOverflowRegions(Rect containerRect, RelativeRect overflow) {
    ...
    if (overflow->left > 0.0) {
        ...
        regions->add(make<_OverflowRegionDataCls>(markerRect, "LEFT OVERFLOWED BY ${_formatPixels(overflow.left)} PIXELS", markerRect->centerLeft + make<OffsetCls>(_indicatorFontSizePixels + _indicatorLabelPaddingPixels, 0.0), math->pi / 2.0, _OverflowSideCls::left));
    }
    if (overflow->right > 0.0) {
        ...
        regions->add(make<_OverflowRegionDataCls>(markerRect, "RIGHT OVERFLOWED BY ${_formatPixels(overflow.right)} PIXELS", markerRect->centerRight - make<OffsetCls>(_indicatorFontSizePixels + _indicatorLabelPaddingPixels, 0.0), -math->pi / 2.0, _OverflowSideCls::right));
    }
    if (overflow->top > 0.0) {
        Rect markerRect = RectCls->fromLTWH(0.0, 0.0, containerRect->width, containerRect->height * _indicatorFraction);
        regions->add(make<_OverflowRegionDataCls>(markerRect, "TOP OVERFLOWED BY ${_formatPixels(overflow.top)} PIXELS", markerRect->topCenter + make<OffsetCls>(0.0, _indicatorLabelPaddingPixels), _OverflowSideCls::top));
    }
    if (overflow->bottom > 0.0) {
        ...
        regions->add(make<_OverflowRegionDataCls>(markerRect, "BOTTOM OVERFLOWED BY ${_formatPixels(overflow.bottom)} PIXELS", markerRect->bottomCenter - make<OffsetCls>(0.0, _indicatorFontSizePixels + _indicatorLabelPaddingPixels), _OverflowSideCls::bottom));
    }
    return regions;
}
...
void DebugOverflowIndicatorMixinCls::_reportOverflow(RelativeRect overflow, List<DiagnosticsNode> overflowHints) {
    ...
    if (overflowHints->isEmpty) {
        overflowHints->add(make<ErrorDescriptionCls>("The edge of the $runtimeType that is overflowing has been marked in the rendering with a yellow and black striped pattern. This is usually caused by the contents being too big for the $runtimeType."));
        overflowHints->add(make<ErrorHintCls>("This is considered an error condition because it indicates that there is content that cannot be seen. If the content is legitimately bigger than the available space, consider clipping it with a ClipRect widget before putting it in the $runtimeType, or using a scrollable container, like a ListView."));
    }

...
List<_OverflowRegionData> DebugOverflowIndicatorMixinCls::_calculateOverflowRegions(RelativeRect overflow, Rect containerRect) {
    ...
    if (overflow->left > 0.0) {
        ...
        regions->add(make<_OverflowRegionDataCls>(markerRect, __s("LEFT OVERFLOWED BY %s,"), markerRect->centerLeft() + make<OffsetCls>(_indicatorFontSizePixels + _indicatorLabelPaddingPixels, 0.0), math->pi / 2.0, _OverflowSideCls::left));
    }
    if (overflow->right > 0.0) {
        ...
        regions->add(make<_OverflowRegionDataCls>(markerRect, __s("RIGHT OVERFLOWED BY %s,"), markerRect->centerRight() - make<OffsetCls>(_indicatorFontSizePixels + _indicatorLabelPaddingPixels, 0.0), -math->pi / 2.0, _OverflowSideCls::right));
    }
    if (overflow->top > 0.0) {
        Rect markerRect = RectCls->fromLTWH(0.0, 0.0, containerRect->width(), containerRect->height() * _indicatorFraction);
        regions->add(make<_OverflowRegionDataCls>(markerRect, __s("TOP OVERFLOWED BY %s,"), markerRect->topCenter() + make<OffsetCls>(0.0, _indicatorLabelPaddingPixels), _OverflowSideCls::top));
    }
    if (overflow->bottom > 0.0) {
        ...
        regions->add(make<_OverflowRegionDataCls>(markerRect, __s("BOTTOM OVERFLOWED BY %s,"), markerRect->bottomCenter() - make<OffsetCls>(0.0, _indicatorFontSizePixels + _indicatorLabelPaddingPixels), _OverflowSideCls::bottom));
    }
    return regions;
}
...
void DebugOverflowIndicatorMixinCls::_reportOverflow(RelativeRect overflow, List<DiagnosticsNode> overflowHints) {
    ...
    if (overflowHints->isEmpty()) {
        overflowHints->add(make<ErrorDescriptionCls>(__s("The edge of the %soverflowing has been marked in the rendering with a yellow and black striped pattern. This is usually caused by the contents being too big for the $%s,")));
        overflowHints->add(make<ErrorHintCls>(__s("This is considered an error condition because it indicates that there is content that cannot be seen. If the content is legitimately bigger than the available space, consider clipping it with a ClipRect widget before putting it in the %scontainer, like a ListView.,")));
    }

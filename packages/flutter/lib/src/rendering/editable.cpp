    ...
    for (InlineSpanSemanticsInformation info : _cachedCombinedSemanticsInfos!) {
        ...
        if (info->isPlaceholder) {
            while (children->length > childIndex && children->elementAt(childIndex)->isTagged(make<PlaceholderSpanIndexSemanticsTagCls>(placeholderIndex))) {
                ...
                TextParentData parentData = ((TextParentData)child!->parentData!);
...
bool RenderEditableCls::hitTestChildren(Offset position, BoxHitTestResult result) {
    ...
    if (span != nullptr && span is HitTestTarget) {
        result->add(make<HitTestEntryCls>(((HitTestTarget)span)));
        hitText = true;
    }
    RenderBox child = firstChild;
    int childIndex = 0;
    while (child != nullptr &&  < _textPainter->inlinePlaceholderBoxes!->length) {
        TextParentData textParentData = ((TextParentData)child->parentData!);
...
void RenderEditableCls::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties->add(make<ColorPropertyCls>("cursorColor", cursorColor));
    properties->add(<ValueNotifier<bool>>make<DiagnosticsPropertyCls>("showCursor", showCursor));
    properties->add(make<IntPropertyCls>("maxLines", maxLines));
    properties->add(make<IntPropertyCls>("minLines", minLines));
    properties->add(<bool>make<DiagnosticsPropertyCls>("expands", expandsfalse));
    properties->add(make<ColorPropertyCls>("selectionColor", selectionColor));
    properties->add(make<DoublePropertyCls>("textScaleFactor", textScaleFactor));
    properties->add(<Locale>make<DiagnosticsPropertyCls>("locale", localenullptr));
    properties->add(<TextSelection>make<DiagnosticsPropertyCls>("selection", selection));
...
RenderEditable _RenderEditableCustomPaintCls::parent() {
    return ((RenderEditable)super->parent);
}

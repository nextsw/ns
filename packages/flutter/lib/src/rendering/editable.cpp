...
TextSelectionPointCls::TextSelectionPointCls(Offset point, TextDirection direction) {
    {
...
RenderEditableCls::RenderEditableCls(Color backgroundCursorColor, List<RenderBox> children, Clip clipBehavior, Color cursorColor, double cursorHeight, Offset cursorOffset, Radius cursorRadius, double cursorWidth, double devicePixelRatio, bool enableInteractiveSelection, LayerLink endHandleLayerLink, bool expands, EdgeInsets floatingCursorAddedMargin, bool forceLine, RenderEditablePainter foregroundPainter, bool hasFocus, bool ignorePointer, Locale locale, int maxLines, int minLines, bool obscureText, String obscuringCharacter, ViewportOffset offset, CaretChangedHandler onCaretChanged, bool paintCursorAboveText, RenderEditablePainter painter, Color promptRectColor, TextRange promptRectRange, bool readOnly, TextSelection selection, Color selectionColor, BoxHeightStyle selectionHeightStyle, BoxWidthStyle selectionWidthStyle, ValueNotifier<bool> showCursor, LayerLink startHandleLayerLink, StrutStyle strutStyle, InlineSpan text, TextAlign textAlign, TextDirection textDirection, TextHeightBehavior textHeightBehavior, double textScaleFactor, TextSelectionDelegate textSelectionDelegate, TextWidthBasis textWidthBasis) {
    {
        ...
        _showCursor = showCursor | <bool>make<ValueNotifierCls>(false);
        _maxLines = maxLines;
        _minLines = minLines;
        _expands = expands;
...
void RenderEditableCls::debugAssertLayoutUpToDate() {
    assert(_textLayoutLastMaxWidth == constraints()->maxWidth && _textLayoutLastMinWidth == constraints()->minWidth, __sf("Last width (%s, %s) not the same as max width constraint (%s, %s).", _textLayoutLastMinWidth, _textLayoutLastMaxWidth, constraints()->minWidth, constraints()->maxWidth));
}
        ...
        if (info->isPlaceholder) {
            ...
            if (recognizer != nullptr) {
                if (is<TapGestureRecognizer>(recognizer)) {
                    ...
                    assert(false, __sf("%s is not supported.", recognizer->runtimeType));
                }
;
                };
                }            }
...
bool RenderEditableCls::hitTestChildren(BoxHitTestResult result, Offset position) {
    bool hitText = false;
...
void RenderEditableCls::setFloatingCursor(FloatingCursorDragState state, Offset boundedOffset, TextPosition lastTextPosition, double resetLerpValue) {
...
void RenderEditableCls::_setTextEditingValue(TextEditingValue newValue, SelectionChangedCause cause) {
...
MapEntry<int, Offset> RenderEditableCls::_lineNumberFor(TextPosition startPosition, List<LineMetrics> metrics) {
    ...
    assert(startPosition->offset == 0, __sf("unable to find the line for %s", startPosition));
...
void _TextHighlightPainterCls::paint(Canvas canvas, Size size, RenderEditable renderEditable) {
...
void _FloatingCursorPainterCls::paintRegularCursor(Canvas canvas, RenderEditable renderEditable, Color caretColor, TextPosition textPosition) {
...
void _CompositeRenderEditablePainterCls::paint(Canvas canvas, Size size, RenderEditable renderEditable) {
    for (RenderEditablePainter painter : painters) {

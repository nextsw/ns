...
void PaintingContextCls::updateLayerProperties(RenderObject child) {
    ...
    assert(identical(updatedLayer, childLayer), __sf("%s created a new layer instance %s instead of reusing the existing layer %s. See the documentation of RenderObject.updateCompositedLayer for more information on how to correctly implement this method.", child, updatedLayer, childLayer));
...
String PaintingContextCls::toString() {
    return __sf("%s#%s(layer: %s, canvas bounds: %s)", objectRuntimeType(this, __s("PaintingContext")), hashCode, _containerLayer, estimatedBounds);
}
    ...
    if (!kReleaseMode) {
        ...
        assert([=] () {
            if (debugEnhanceLayoutTimelineArguments) {
                            Map<String, String> map1 = make<MapCls<>>();            map1.set(__s("dirty count"), __sf("%s", _nodesNeedingLayout->length()));            map1.set(__s("dirty list"), __sf("%s", _nodesNeedingLayout));debugTimelineArguments = list1;
            }
            return true;
        }());
    ...
    if (!kReleaseMode) {
        ...
        assert([=] () {
            if (debugEnhancePaintTimelineArguments) {
                            Map<String, String> map1 = make<MapCls<>>();            map1.set(__s("dirty count"), __sf("%s", _nodesNeedingPaint->length()));            map1.set(__s("dirty list"), __sf("%s", _nodesNeedingPaint));debugTimelineArguments = list1;
            }
            return true;
        }());
        ...
        if (owner() != nullptr) {
            assert([=] () {
                if (debugPrintMarkNeedsLayoutStacks) {
                    debugPrintStack(__sf("markNeedsLayout() called for %s", this));
                }
                return true;
            }());
...
void RenderObjectCls::layout(Constraints constraints, bool parentUsesSize) {
    ...
    if (!kReleaseMode && debugProfileLayoutsEnabled) {
        ...
        TimelineCls->startSync(__sf("%s", runtimeType), debugTimelineArguments);
    }
    ...
    assert([=] () {
        ...
        if (debugPrintLayouts) {
            debugPrint(__sf("Laying out (%s", sizedByParent()? __s("with separate resize") : __sf("with resize allowed) %s", this)));
        }
        return true;
    }());
    if (sizedByParent()) {
    ...
    if (isRepaintBoundary() && _wasRepaintBoundary) {
        assert([=] () {
            if (debugPrintMarkNeedsPaintStacks) {
                debugPrintStack(__sf("markNeedsPaint() called for %s", this));
            }
            return true;
        }());
        ...
        assert([=] () {
            if (debugPrintMarkNeedsPaintStacks) {
                debugPrintStack(__sf("markNeedsPaint() called for %s (root of render tree)", this));
            }
            return true;
        }());
        if (owner() != nullptr) {
...
String RenderObjectCls::toStringShort() {
    ...
    if (!kReleaseMode) {
        ...
        if (_relayoutBoundary != nullptr && _relayoutBoundary != this) {
            ...
            header += __sf(" relayoutBoundary=up%s", count);
        }
        if (_needsLayout) {
...
void RenderObjectCls::_debugReportException(String method, Object exception, StackTrace stack) {
    List<DiagnosticsNode> list1 = make<ListCls<>>();if (kDebugMode && debugCreator != nullptr) {    list1.add(ArrayItem);}list1.add(ArrayItem);list1.add(ArrayItem);FlutterErrorCls->reportError(make<FlutterErrorDetailsCls>(exception, stack, __s("rendering library"), make<ErrorDescriptionCls>(__sf("during %s()", method)), [=] () {
        list1;
    }));
}
...
bool RenderObjectCls::_debugCanPerformMutations() {
    ...
    assert([=] () {
        ...
        String culpritFullMethodName = __sf("%s.%s", debugActiveLayout->runtimeType, culpritMethodName);
        result = false;
        ...
        ErrorSummary summary = make<ErrorSummaryCls>(__sf("A %s was mutated in %s.", runtimeType, culpritFullMethodName));
        bool isMutatedByAncestor = activeLayoutRoot == debugActiveLayout;
        String description = isMutatedByAncestor? __sf("A RenderObject must not mutate its descendants in its %s method.", culpritMethodName) : __sf("A RenderObject must not mutate another RenderObject from a different render subtree in its %s method.", culpritMethodName);
...
void RenderObjectCls::_layoutWithoutResize() {
    ...
    assert([=] () {
        ...
        _debugActiveLayout = this;
        if (debugPrintLayouts) {
            debugPrint(__sf("Laying out (without resize) %s", this));
        }
        return true;
    }());
    try {
...
_SemanticsFragment RenderObjectCls::_getSemanticsForParent(bool mergeIntoParent) {
    ...
    assert(!_needsLayout, __sf("Updated layout information required for %s to calculate semantics.", this));

...
String RelativeRectCls::toString() {
    return __sf("RelativeRect.fromLTRB(%s, %s, %s, %s)", left->toStringAsFixed(1), top->toStringAsFixed(1), right->toStringAsFixed(1), bottom->toStringAsFixed(1));
}
...
List<DiagnosticsNode> RenderIndexedStackCls::debugDescribeChildren() {
    ...
    while (child != nullptr) {
        children->add(child->toDiagnosticsNode(__sf("child %s", i + 1), i != index()!? DiagnosticsTreeStyleCls::offstage : nullptr));

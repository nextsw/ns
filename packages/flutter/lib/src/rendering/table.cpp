...
void RenderTableCls::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties->add(<TableBorder>make<DiagnosticsPropertyCls>("border", bordernullptr));
    properties->add(<Map<int, TableColumnWidth>>make<DiagnosticsPropertyCls>("specified column widths", _columnWidths_columnWidths->isEmpty? DiagnosticLevelCls::hidden : DiagnosticLevelCls::info));
    properties->add(<TableColumnWidth>make<DiagnosticsPropertyCls>("default column width", defaultColumnWidth));
    properties->add(make<MessagePropertyCls>("table size", "$columns\u00D7$rows"));
    properties->add(<String>make<IterablePropertyCls>("column offsets", _columnLefts?->map(debugFormatDouble)"unknown"));
    properties->add(<String>make<IterablePropertyCls>("row offsets", _rowTops->map(debugFormatDouble)"unknown"));
}
    ...
    for (;  < rows; y = 1) {
        for (;  < columns; x = 1) {
            ...
            if (child != nullptr) {
                ...
                children->add(<Object>make<DiagnosticsPropertyCls>(name, nullptr"is null", false));
            }
        }
    }
    return children;

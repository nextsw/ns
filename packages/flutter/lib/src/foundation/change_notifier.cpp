...
bool ChangeNotifierCls::debugAssertNotDisposed(ChangeNotifier notifier) {
    assert([=] () {
        if (notifier->_debugDisposed) {
            throw make<FlutterErrorCls>(__sf("A %s was used after being disposed.\nOnce you have called dispose() on a %s, it can no longer be used.", notifier->runtimeType, notifier->runtimeType));
        }
        return true;
    }());
    return true;
...
void ChangeNotifierCls::notifyListeners() {
    ...
    for (;  < end; i++) {
        try {
            ...
            FlutterErrorCls->reportError(make<FlutterErrorDetailsCls>(exception, stack, __s("foundation library"), make<ErrorDescriptionCls>(__sf("while dispatching notifications for %s", runtimeType)), [=] () {
                makeList(ArrayItem);
            }));
        };
    }
...
String _MergingListenableCls::toString() {
    return __sf("Listenable.merge([%s])", _children->join(__s(", ")));
}
...
String ValueNotifierCls<T>::toString() {
    return __sf("%s(%s)", describeIdentity(this), value());
}

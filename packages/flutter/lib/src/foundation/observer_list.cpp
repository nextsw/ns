...
template<typename T>
void ObserverListCls<T>::add(T item) {
    _isDirty = true;
    _list->add(item);
}

template<typename T>
bool ObserverListCls<T>::remove(T item) {
    _isDirty = true;
    _set->clear();
...
template<typename T>
bool ObserverListCls<T>::contains(Object element) {
    if (_list->length() < 3) {
...
template<typename T>
List<T> ObserverListCls<T>::toList(bool growable) {
...
template<typename T>
bool HashedObserverListCls<T>::remove(T item) {
    int value = _map[item];
    if (value == nullptr) {
        return false;
    }
...
template<typename T>
bool HashedObserverListCls<T>::contains(Object element) {

...
void EditableTextCls::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties->add(<TextEditingController>make<DiagnosticsPropertyCls>("controller", controller));
    properties->add(<FocusNode>make<DiagnosticsPropertyCls>("focusNode", focusNode));
    properties->add(<bool>make<DiagnosticsPropertyCls>("obscureText", obscureTextfalse));
    properties->add(<bool>make<DiagnosticsPropertyCls>("readOnly", readOnlyfalse));
    properties->add(<bool>make<DiagnosticsPropertyCls>("autocorrect", autocorrecttrue));
    properties->add(<SmartDashesType>make<EnumPropertyCls>("smartDashesType", smartDashesTypeobscureText? SmartDashesTypeCls::disabled : SmartDashesTypeCls::enabled));
    properties->add(<SmartQuotesType>make<EnumPropertyCls>("smartQuotesType", smartQuotesTypeobscureText? SmartQuotesTypeCls::disabled : SmartQuotesTypeCls::enabled));
    properties->add(<bool>make<DiagnosticsPropertyCls>("enableSuggestions", enableSuggestionstrue));
    ...
    properties->add(make<IntPropertyCls>("maxLines", maxLines1));
    properties->add(make<IntPropertyCls>("minLines", minLinesnullptr));
    properties->add(<bool>make<DiagnosticsPropertyCls>("expands", expandsfalse));
    properties->add(<bool>make<DiagnosticsPropertyCls>("autofocus", autofocusfalse));
    properties->add(<TextInputType>make<DiagnosticsPropertyCls>("keyboardType", keyboardTypenullptr));
    properties->add(<ScrollController>make<DiagnosticsPropertyCls>("scrollController", scrollControllernullptr));
    properties->add(<ScrollPhysics>make<DiagnosticsPropertyCls>("scrollPhysics", scrollPhysicsnullptr));
    properties->add(<Iterable<String>>make<DiagnosticsPropertyCls>("autofillHints", autofillHintsnullptr));
    properties->add(<TextHeightBehavior>make<DiagnosticsPropertyCls>("textHeightBehavior", textHeightBehaviornullptr));
    properties->add(<bool>make<DiagnosticsPropertyCls>("scribbleEnabled", scribbleEnabledtrue));
    properties->add(<bool>make<DiagnosticsPropertyCls>("enableIMEPersonalizedLearning", enableIMEPersonalizedLearningtrue));
    properties->add(<bool>make<DiagnosticsPropertyCls>("enableInteractiveSelection", enableInteractiveSelectiontrue));
}
...
void EditableTextStateCls::cutSelection(SelectionChangedCause cause) {
    ...
    ClipboardCls->setData(make<ClipboardDataCls>(selection->textInside(text)));
    _replaceText(make<ReplaceTextIntentCls>(textEditingValue, "", selection, cause));
    if (cause == SelectionChangedCauseCls::toolbar) {
...
RenderEditable EditableTextStateCls::renderEditable() {
    return ((RenderEditable)_editableKey->currentContext!->findRenderObject()!);
}
...
void EditableTextStateCls::_finalizeEditing(TextInputAction action, bool shouldUnfocus) {
    if (widget->onEditingComplete != nullptr) {
        try {
            ...
            FlutterErrorCls->reportError(make<FlutterErrorDetailsCls>(exception, stack, "widgets", make<ErrorDescriptionCls>("while calling onEditingComplete for $action")));
        };
    } else {
    ...
    try {
        ...
        FlutterErrorCls->reportError(make<FlutterErrorDetailsCls>(exception, stack, "widgets", make<ErrorDescriptionCls>("while calling onSubmitted for $action")));
    };
    if (shouldUnfocus) {
...
void EditableTextStateCls::_handleSelectionChanged(SelectionChangedCause cause, TextSelection selection) {
    ...
    try {
        ...
        FlutterErrorCls->reportError(make<FlutterErrorDetailsCls>(exception, stack, "widgets", make<ErrorDescriptionCls>("while calling onSelectionChanged for $cause")));
    };
...
void EditableTextStateCls::_formatAndSetValue(SelectionChangedCause cause, bool userInteraction, TextEditingValue value) {
    ...
    if (textChanged) {
        try {
            ...
            FlutterErrorCls->reportError(make<FlutterErrorDetailsCls>(exception, stack, "widgets", make<ErrorDescriptionCls>("while applying input formatters")));
        };
    }
    beginBatchEdit();
    _value = value;
    ...
    if (textChanged) {
        try {
            ...
            FlutterErrorCls->reportError(make<FlutterErrorDetailsCls>(exception, stack, "widgets", make<ErrorDescriptionCls>("while calling onChanged")));
        };
    }
    endBatchEdit();
}
...
Action<T> EditableTextStateCls::_makeOverridabletemplate<typename T : Intent> (Action<T> defaultAction) {
...
RenderEditable _ScribbleFocusableStateCls::renderEditable() {
    return ((RenderEditable)widget->editableKey->currentContext?->findRenderObject());
}
...
template<typename T : DirectionalTextEditingIntent> Object _DeleteTextActionCls<T>::invoke(BuildContext context, T intent) {
    TextSelection selection = state->_value->selection;
    assert(selection->isValid);
    if (!selection->isCollapsed) {
        return ActionsCls->invoke(context!, make<ReplaceTextIntentCls>(state->_value, "", _expandNonCollapsedRange(state->_value), SelectionChangedCauseCls::keyboard));
    }
    ...
    if (!textBoundary->textEditingValue->selection->isCollapsed) {
        return ActionsCls->invoke(context!, make<ReplaceTextIntentCls>(state->_value, "", _expandNonCollapsedRange(textBoundary->textEditingValue), SelectionChangedCauseCls::keyboard));
    }
    return ActionsCls->invoke(context!, make<ReplaceTextIntentCls>(textBoundary->textEditingValue, "", textBoundary->getTextBoundaryAt(textBoundary->textEditingValue->selection->base), SelectionChangedCauseCls::keyboard));
}

template<typename T : DirectionalTextEditingIntent> bool _DeleteTextActionCls<T>::isActionEnabled() {
    return !state->widget->readOnly && state->_value->selection->isValid;
}

template<typename T : DirectionalTextEditingIntent> TextRange _DeleteTextActionCls<T>::_expandNonCollapsedRange(TextEditingValue value) {
...
template<typename T : DirectionalCaretMovementIntent> Object _UpdateTextSelectionActionCls<T>::invoke(BuildContext context, T intent) {
    TextSelection selection = state->_value->selection;
    assert(selection->isValid);
...
template<typename T : DirectionalCaretMovementIntent> bool _UpdateTextSelectionActionCls<T>::isActionEnabled() {
    return state->_value->selection->isValid;
}

template<typename T : DirectionalCaretMovementIntent> bool _UpdateTextSelectionActionCls<T>::_isAtWordwrapUpstream(TextPosition position) {
...
template<typename T : DirectionalCaretMovementIntent> bool _UpdateTextSelectionActionCls<T>::_isAtWordwrapDownstream(TextPosition position) {
...
template<typename T : DirectionalCaretMovementIntent> void _UpdateTextSelectionToAdjacentLineActionCls<T>::stopCurrentVerticalRunIfSelectionChanges() {
...
template<typename T : DirectionalCaretMovementIntent> void _UpdateTextSelectionToAdjacentLineActionCls<T>::invoke(BuildContext context, T intent) {
...
template<typename T : DirectionalCaretMovementIntent> bool _UpdateTextSelectionToAdjacentLineActionCls<T>::isActionEnabled() {
    return state->_value->selection->isValid;
}

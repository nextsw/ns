...
SliverReorderableListCls::SliverReorderableListCls(ChildIndexGetter findChildIndexCallback, IndexedWidgetBuilder itemBuilder, int itemCount, double itemExtent, Key key, ReorderCallback onReorder, std::function<void(int )> onReorderEnd, std::function<void(int )> onReorderStart, Widget prototypeItem, ReorderItemProxyDecorator proxyDecorator) {
    {
        assert(itemCount >= 0);
        assert(itemExtent == nullptr || prototypeItem == nullptr, __s("You can only pass itemExtent or prototypeItem, not both"));
    }

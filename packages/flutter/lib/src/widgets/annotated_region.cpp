...
template<typename T>
RenderObject AnnotatedRegionCls<T>::createRenderObject(BuildContext context) {
    return <T>make<RenderAnnotatedRegionCls>(value, sized);
}

template<typename T>
void AnnotatedRegionCls<T>::updateRenderObject(BuildContext context, RenderAnnotatedRegion<T> renderObject) {

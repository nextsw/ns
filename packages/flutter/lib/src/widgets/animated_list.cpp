...
void SliverAnimatedListStateCls::insertItem(Duration duration, int index) {
...
void SliverAnimatedListStateCls::removeItem(AnimatedListRemovedItemBuilder builder, Duration duration, int index) {
    assert(index != nullptr && index >= 0);
...
_ActiveItem SliverAnimatedListStateCls::_removeActiveItemAt(int itemIndex, List<_ActiveItem> items) {
    int i = binarySearch(items, _ActiveItemCls->index(itemIndex));
    return i == -1? nullptr : items->removeAt(i);
}

_ActiveItem SliverAnimatedListStateCls::_activeItemAt(int itemIndex, List<_ActiveItem> items) {
    int i = binarySearch(items, _ActiveItemCls->index(itemIndex));

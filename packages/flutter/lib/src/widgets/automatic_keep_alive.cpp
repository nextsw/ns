...
void _AutomaticKeepAliveStateCls::debugFillProperties(DiagnosticPropertiesBuilder description) {
    ...
    description->add(<Map<Listenable, VoidCallback>>make<DiagnosticsPropertyCls>(__s("handles"), _handles, _handles != nullptr? __sf("%s active client%s", _handles!->length(), _handles!->length() == 1? __s("") : __s("s")) : nullptr, __s("no notifications ever received")));
}

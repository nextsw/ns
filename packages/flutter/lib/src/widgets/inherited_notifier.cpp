...
template<typename T>
bool InheritedNotifierCls<T>::updateShouldNotify(InheritedNotifier<T> oldWidget) {
    return oldWidget->notifier != notifier;
}

template<typename T>
InheritedElement InheritedNotifierCls<T>::createElement() {
...
template<typename T>
void _InheritedNotifierElementCls<T>::notifyClients(InheritedNotifier<T> oldWidget) {
...
template<typename T>
void _InheritedNotifierElementCls<T>::_handleUpdate() {
    _dirty = true;
    markNeedsBuild();
}

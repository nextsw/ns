...
void SliverChildDelegateCls::debugFillDescription(List<String> description) {
    try {
        ...
        if (children != nullptr) {
            description->add(__sf("estimated child count: %s", children));
        }
    } catch (Unknown e) {
        description->add(__sf("estimated child count: EXCEPTION (%s)", e->runtimeType()));
    };
}
...
SliverChildBuilderDelegateCls::SliverChildBuilderDelegateCls(NullableIndexedWidgetBuilder builder, bool addAutomaticKeepAlives, bool addRepaintBoundaries, bool addSemanticIndexes, int childCount, ChildIndexGetter findChildIndexCallback, SemanticIndexCallback semanticIndexCallback, int semanticIndexOffset) {
    {
...
int SliverChildBuilderDelegateCls::findIndexByKey(Key key) {
    ...
    if (is<_SaltedValueKey>(key)) {
        _SaltedValueKey saltedValueKey = as<_SaltedValueKeyCls>(key);
...
SliverChildListDelegateCls::SliverChildListDelegateCls(List<Widget> children, bool addAutomaticKeepAlives, bool addRepaintBoundaries, bool addSemanticIndexes, SemanticIndexCallback semanticIndexCallback, int semanticIndexOffset) {
    {
...
Widget SliverChildListDelegateCls::build(BuildContext context, int index) {
    ...
    Key key = child->key != nullptr? make<_SaltedValueKeyCls>(child->key!) : nullptr;
    assert(child != nullptr, __sf("The sliver's children must not contain null values, but a null value was found at index %s", index));
    if (addRepaintBoundaries) {
        child = make<RepaintBoundaryCls>(child);
    }
    if (addSemanticIndexes) {
...
Element SliverMultiBoxAdaptorElementCls::updateChild(Element child, Widget newWidget, Object newSlot) {
    ...
    if (result == nullptr) {
        ...
        while (_build(hi - 1, adaptorWidget) != nullptr) {
            ...
            if ( < max ~/ 2) {
                ...
                throw make<FlutterErrorCls>(__sf("Could not find the number of children in %s.\nThe childCount getter was called (implying that the delegate's builder returned null for a positive index), but even building the child with index %s (the maximum possible integer) did not return null. Consider implementing childCount to avoid the cost of searching for the final child.", adaptorWidget->delegate, hi));
            }
;
            }        }
        while (hi - lo > 1) {
...
double SliverMultiBoxAdaptorElementCls::_extrapolateMaxScrollOffset(int firstIndex, int lastIndex, double leadingScrollOffset, double trailingScrollOffset, int childCount) {

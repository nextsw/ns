...
List<FocusNode> FocusTraversalPolicyCls::_sortAllDescendants(FocusScopeNode scope, FocusNode currentNode) {
    ...
    assert(sortedDescendants->length() <= scope->traversalDescendants()->length() && sortedDescendants->toSet()->difference(scope->traversalDescendants()->toSet())->isEmpty, __sf("Sorted descendants contains different nodes than FocusScopeNode.traversalDescendants would. These are the different nodes: %s", sortedDescendants->toSet()->difference(scope->traversalDescendants()->toSet())));
...
void _ReadingOrderDirectionalGroupDataCls::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties->add(<String>make<IterablePropertyCls>(__s("members"), members-><String>map([=] (_ReadingOrderSortData member) {
        return __sf(""%s"(%s)", member->node->debugLabel, member->rect);
    })));
}
...
int FocusOrderCls::compareTo(FocusOrder other) {
    assert(runtimeType == other->runtimeType, __sf("The sorting algorithm must not compare incomparable keys, since they don't know how to order themselves relative to each other. Comparing %s with %s", this, other));
...
Iterable<FocusNode> OrderedTraversalPolicyCls::sortDescendants(Iterable<FocusNode> descendants, FocusNode currentNode) {
    ...
    <_OrderedFocusInfo>mergeSort(ordered, [=] (_OrderedFocusInfo a,_OrderedFocusInfo b) {
        assert(a->order->runtimeType() == b->order->runtimeType(), __sf("When sorting nodes for determining focus order, the order (%s) of node %s, isn't the same type as the order (%s) of %s. Incompatible order types can't be compared.  Use a FocusTraversalGroup to group similar orders together.", a->order, a->node, b->order, b->node));
...
FocusOrder FocusTraversalOrderCls::of(BuildContext context) {
    ...
    assert([=] () {
        if (marker == nullptr) {
            throw make<FlutterErrorCls>(__sf("FocusTraversalOrder.of() was called with a context that does not contain a FocusTraversalOrder widget. No TraversalOrder widget ancestor could be found starting from the context that was passed to FocusTraversalOrder.of().\nThe context used was:\n  %s", context));
        }
        return true;
    }());
    return marker!->order;

...
void _ModalScopeStatusCls::debugFillProperties(DiagnosticPropertiesBuilder description) {
    ...
    description->add(make<FlagPropertyCls>("isCurrent"isCurrent, "active", "inactive"));
    description->add(make<FlagPropertyCls>("canPop"canPop, "can pop"));
    description->add(make<FlagPropertyCls>("impliesAppBarDismissal"impliesAppBarDismissal, "implies app bar dismissal"));
}
...
template<typename T> ModalRoute<T> ModalRouteCls<T>::oftemplate<typename T : Object> (BuildContext context) {
...
template<typename R : Route<dynamic>> bool RouteObserverCls<R>::debugObservingRoute(R route) {
    bool contained;
    assert([=] () {
...
template<typename R : Route<dynamic>> void RouteObserverCls<R>::subscribe(R route, RouteAware routeAware) {
...
template<typename R : Route<dynamic>> void RouteObserverCls<R>::unsubscribe(RouteAware routeAware) {
    assert(routeAware != nullptr);
...
template<typename R : Route<dynamic>> void RouteObserverCls<R>::didPop(Route<dynamic> previousRoute, Route<dynamic> route) {
...
template<typename R : Route<dynamic>> void RouteObserverCls<R>::didPush(Route<dynamic> previousRoute, Route<dynamic> route) {
    if (route is R && previousRoute is R) {
...
void _RenderFocusTrapCls::handleEvent(HitTestEntry entry, PointerEvent event) {
    ...
    if (event is! PointerDownEvent || event->buttons != kPrimaryButton || event->kind != PointerDeviceKindCls::mouse || _shouldIgnoreEvents || _focusScopeNode->focusedChild == nullptr) {
        return;
    }

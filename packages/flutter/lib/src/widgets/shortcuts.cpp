...
template<typename T : KeyboardKey> KeySetCls<T>::KeySetCls(T key1, T key2, T key3, T key4) {
    {
        assert(key1 != nullptr);
...
template<typename T : KeyboardKey> void KeySetCls<T>::fromSet(Set<T> keys)

template<typename T : KeyboardKey> Set<T> KeySetCls<T>::keys() {
    return _keys->toSet();
}

template<typename T : KeyboardKey> bool KeySetCls<T>::==(Object other) {
...
template<typename T : KeyboardKey> int KeySetCls<T>::_computeHashCodetemplate<typename T> (Set<T> keys) {
...
void LogicalKeySetCls::debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties->add(<Set<LogicalKeyboardKey>>make<DiagnosticsPropertyCls>("keys", _keysdebugDescribeKeys()));
}
